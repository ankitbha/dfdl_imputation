...
def load_transformer_data_for_contrastive(exp_file, train_file, test_file, val_file):
    ...
    # Sort data by label (separating positive and negative examples)
    ...
    # Get positive/negative split points
    train_pos_idx = np.sum(train_data[:,2])
    val_pos_idx = np.sum(valid_data[:,2])
    test_pos_idx = np.sum(test_data[:,2])
    
    # Create adjacency matrices in format needed for contrastive learning
...
def evaluate_contrastive_model(model, embeddings, adjacency_matrix, loss_function, device):
    ...
    with torch.no_grad():
        ...
        metrics['auc_pr'] = average_precision_score(true_labels, probabilities)
...
def run_experiment(gpu_id, dataset_id, train_ratio, embeddings,
    ...
    if True:
        data_dict = load_transformer_data_for_contrastive(
            exp_file='../bni/GNNLink/Data/hESC/TFs+500/BL--ExpressionData.csv',
            train_file='../bni/GNNLink/Data/Train_validation_test/hESC 500/Train_set.csv',
            test_file='../bni/GNNLink/Data/Train_validation_test/hESC 500/Test_set.csv',
            val_file='../bni/GNNLink/Data/Train_validation_test/hESC 500/Validation_set.csv'
        )

        # Get features and adjacency matrices
        embeddings = data_dict['features']
        # embeddings = PCA_embeddings(features, n_components=64)
