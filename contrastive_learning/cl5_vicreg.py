...
class GraphAugmentor:
    ...
    def edge_removing(self, p: float = 0.1) -> np.ndarray:
        ...
        Args:
            p: Probability of removing each known edge
    ...
    def edge_adding(self, p: float = 0.1) -> np.ndarray:
        ...
        Args:
            p: Ratio of new edges to add relative to existing edges
        ...
        # Find potential edges to add (currently 0 or -1)
    ...
    def edge_flipping(self, p: float = 0.1) -> np.ndarray:
        ...
        Args:
            p: Probability of flipping each known edge
        ...
        for i, j in edges_to_flip:
            adj[i, j] = 1 - adj[i, j]  # Flip 0 to 1 or 1 to 0
        return adj
    ...
    def random_walk_subgraph(self, 
                            walk_length: int = 10, 
                            num_walks: int = 100) -> np.ndarray:
        """
        ...
        Returns:
            Augmented adjacency matrix
        """
        adj = np.zeros_like(self.adj_matrix)
        ...
        for start_node in start_nodes:
            ...
            for _ in range(walk_length):
                ...
                adj[current, next_node] = self.adj_matrix[current, next_node]
                adj[next_node, current] = self.adj_matrix[next_node, current]
                current = next_node
    ...
    def personalized_pagerank(self, 
        ...
        deg = np.sum(self.binary_adj, axis=1)
        deg_inv = np.power(deg, -1)
        deg_inv[np.isinf(deg_inv)] = 0
        norm_adj = self.binary_adj * deg_inv[:, np.newaxis]
        
        # Initialize PPR matrix
        ppr = np.eye(self.num_nodes)
        prev_ppr = np.zeros_like(ppr)
        
        # Power iteration
        while np.abs(ppr - prev_ppr).sum() > epsilon:
            prev_ppr = ppr.copy()
            ppr = (1 - alpha) * np.dot(norm_adj, ppr) + alpha * np.eye(self.num_nodes)
        
        # Apply PPR scores to original adjacency
        return ppr * self.adj_matrix
    
    def markov_diffusion(self, 
        ...
        norm_adj = (self.binary_adj * 
                   deg_inv_sqrt[:, np.newaxis] * 
                   deg_inv_sqrt[np.newaxis, :])
        lap = np.eye(self.num_nodes) - norm_adj
        
        # Compute diffusion kernel
        kernel = expm(-t * lap)
        
        # Apply kernel scores to original adjacency
        return kernel * self.adj_matrix
...
def run_augmentation_experiments(dataset_id: int,
    ...
    for aug_name, params in topology_augs.items():
        for param_values in [dict(zip(params.keys(), v)) 
    ...
    for aug_name, params in feature_augs.items():
        for param_values in [dict(zip(params.keys(), v)) 
                           for v in product(*params.values())]:
            config = {
                'name': f"{aug_name}",
...
class PredictionHead(nn.Module):
    """MLP prediction head for Bootstrapping Latent loss"""
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
...
class ContrastiveModel(nn.Module):
    def __init__(
        ...
        vicreg_params: dict = {'sim_coeff': 25.0, 'std_coeff': 25.0, 'cov_coeff': 1.0},
        momentum: float = 0.99,
        **kwargs  # Add this to capture any additional parameters
    ):
        super().__init__()
        ...
        if loss_type == 'bt':
            self.bt_lambda = kwargs.get('bt_lambda', self.bt_lambda)
        if loss_type == 'vicreg':
            self.vicreg_params = {
                'sim_coeff': kwargs.get('sim_coeff', self.vicreg_params['sim_coeff']),
                'std_coeff': kwargs.get('std_coeff', self.vicreg_params['std_coeff']),
                'cov_coeff': kwargs.get('cov_coeff', self.vicreg_params['cov_coeff'])
            }
        
        # Base encoder - maintains consistent dimensions
        ...
        # Projection head - from projection_dim to projection_dim
        ...
        # For BL loss
        if loss_type == 'bl':
            self.encoder_momentum = nn.Sequential(
                nn.Linear(input_dim, hidden_dim),
                nn.BatchNorm1d(hidden_dim),
                nn.ReLU(),
                nn.Linear(hidden_dim, projection_dim),
    ...
    def _bootstrapping_latent_loss(self, x1: torch.Tensor, x2: torch.Tensor) -> torch.Tensor:
        p1 = self.predictor(self.projection(self.encoder(x1)))
        p2 = self.predictor(self.projection(self.encoder(x2)))

        with torch.no_grad():
            z1_momentum = self.projection(self.encoder_momentum(x1))
            z2_momentum = self.projection(self.encoder_momentum(x2))
        ...
        loss = -0.5 * (
            F.cosine_similarity(p1, z2_momentum.detach(), dim=1).mean() +
            F.cosine_similarity(p2, z1_momentum.detach(), dim=1).mean()
        )
    ...
    def _update_momentum_encoder(self):
        ...
        for param_q, param_k in zip(self.encoder.parameters(), self.encoder_momentum.parameters()):
            param_k.data = param_k.data * self.momentum_value + param_q.data * (1.0 - self.momentum_value)

    def compute_loss(self, x1: torch.Tensor, x2: torch.Tensor, **kwargs) -> torch.Tensor:
        if self.loss_type == 'bl':
            return self._bootstrapping_latent_loss(x1, x2)
        else:
            z1, z2 = self.forward(x1, x2)
            if self.loss_type == 'infonce':
                return self._infonce_loss(z1, z2, **kwargs)
            elif self.loss_type == 'jsd':
                return self._jsd_loss(z1, z2, **kwargs)
            elif self.loss_type == 'tm':
                return self._triplet_margin_loss(z1, z2, **kwargs)
            elif self.loss_type == 'bt':
                return self._barlow_twins_loss(z1, z2, **kwargs)
            elif self.loss_type == 'vicreg':
                return self._vicreg_loss(z1, z2, **kwargs)
            else:
    ...
    def _jsd_loss(self, z1: torch.Tensor, z2: torch.Tensor) -> torch.Tensor:
        """Jensen-Shannon Divergence loss with softplus"""
        batch_size = z1.shape[0]
        z1 = F.normalize(z1, dim=1)
        z2 = F.normalize(z2, dim=1)
        
        scores_pos = self.discriminator(z1 * z2)
        scores_neg = self.discriminator(z1 * z2[torch.randperm(batch_size)])
    ...
    def _triplet_margin_loss(self, z1: torch.Tensor, z2: torch.Tensor) -> torch.Tensor:
        """Triplet Margin loss"""
        batch_size = z1.shape[0]
        z1 = F.normalize(z1, dim=1)
        z2 = F.normalize(z2, dim=1)
        ...
        # Positive pairs
        pos_dist = torch.norm(z1 - z2, dim=1)
        
        # Negative pairs
        z2_neg = z2[torch.randperm(batch_size)]
        neg_dist = torch.norm(z1 - z2_neg, dim=1)
    ...
    def _barlow_twins_loss(self, z1: torch.Tensor, z2: torch.Tensor) -> torch.Tensor:
        ...
        off_diag = torch.triu(c.pow_(2), diagonal=1)
        
        loss = on_diag.sum() + self.bt_lambda * off_diag.sum()
        return loss
    ...
    def _vicreg_loss(self, z1: torch.Tensor, z2: torch.Tensor) -> torch.Tensor:
        ...
        loss = self.vicreg_params['sim_coeff'] * sim_loss + \
               self.vicreg_params['std_coeff'] * std_loss + \
               self.vicreg_params['cov_coeff'] * cov_loss
        return loss
...
def run_contrastive_experiments(
    ...
    """Run experiments with different contrastive objectives"""
    results = []
    
    loss_configs = {
        # 'infonce': {'temperature': [1.0, 10, 100]},
        # 'jsd': {'temperature': [1.0, 10, 100]},
        # 'tm': {'margin': [0.5, 1.0, 2.0]},
        # 'bl': {'momentum': [0.9, 0.99, 0.999]},
        # 'bt': {'bt_lambda': [0.001, 0.005, 0.01]},
        'vicreg': {
            # 'sim_coeff': [10.0, 25.0],
            # 'std_coeff': [10.0, 25.0],
            # 'cov_coeff': [0.5, 1.0]
    ...
    for loss_type, params in tqdm(loss_configs.items()):
        param_combinations = [dict(zip(params.keys(), v)) 
                            for v in product(*params.values())]
        
        for param_dict in tqdm(param_combinations):
            config = {
                'loss_type': loss_type,
                'params': param_dict
            }
            ...
            metrics = []
            for run in range(n_runs):
                # Create model with current loss configuration
                model = ContrastiveModel(
                    input_dim=base_params['input_dim'],
                    hidden_dim=base_params['hidden_dim'],
                    projection_dim=base_params['output_dim'],
                    loss_type=loss_type,
                    **param_dict
                ).to(device)
                ...
                for epoch in range(base_params['num_epochs']):
                    for batch_x1, batch_x2, _ in generate_batches(
                        embeddings, adj_matrix,
                        base_params['batch_size'],
                        base_params['num_batches'],
                        base_params['negative_ratio']
                    ):
                        batch_x1 = batch_x1.to(device, dtype=torch.float32)
                        batch_x2 = batch_x2.to(device, dtype=torch.float32)
                        
                        optimizer.zero_grad()
                        loss = model.compute_loss(batch_x1, batch_x2)
                        loss.backward()
                        optimizer.step()
                        
                        if loss_type == 'bl':
                            # Update momentum encoder for BL loss
                            model._update_momentum_encoder()
                        print(f"Epoch {epoch+1} - Loss: {loss.item()}")
                
                # Evaluate
                run_metrics = evaluate_contrastive_model(
                    model, embeddings, adj_matrix, loss_type, device
                )
                print(f"Run {run+1} - {loss_type} - {params} - {param_dict}: {run_metrics}")
                metrics.append(run_metrics)
...
def plot_contrastive_results(results: list, save_dir: str):
    """Plot results from contrastive objective experiments"""
    os.makedirs(save_dir, exist_ok=True)
    ...
    for result in results:
        ...
        param_str = '_'.join(f"{k}={v:.3f}" for k, v in params.items())
        
        df_data.append({
            'Loss': f"{loss_type}_{param_str}",
            'AUC-ROC': result['metrics']['auc_roc'],
            'AUC-PR': result['metrics']['auc_pr'],
            'F1': result['metrics']['f1_score'],
            'MCC': result['metrics']['mcc']
    ...
    sns.barplot(data=df, x='Loss', y='AUC-ROC')
    plt.xticks(rotation=45, ha='right')
    plt.title('Contrastive Objectives Comparison - AUC-ROC')
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'contrastive_objectives_auc_roc.png'))
    plt.close()
    
    # Plot all metrics
    metrics = ['AUC-ROC', 'AUC-PR', 'F1', 'MCC']
    df_melted = df.melt(id_vars=['Loss'], value_vars=metrics, 
                        var_name='Metric', value_name='Value')
    
    plt.figure(figsize=(15, 10))
    sns.barplot(data=df_melted, x='Loss', y='Value', hue='Metric')
    plt.xticks(rotation=45, ha='right')
    plt.title('Contrastive Objectives Comparison - All Metrics')
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'contrastive_objectives_all_metrics.png'))
    plt.close()
    
    # Save numerical results
    df.to_csv(os.path.join(save_dir, 'contrastive_objectives_results.csv'), index=False)
...
def evaluate_contrastive_model(model, embeddings, adjacency_matrix, loss_function, device):
    ...
    with torch.no_grad():
        ...
        encoded = model.encoder(embeddings_tensor)
        # Then get projection
        projected_embeddings = model.projection(encoded)
        ...
        print("true_labels_shape: ", true_labels.shape)
        print("probabilities: ", probabilities)
        print("probabilities_shape: ", probabilities.shape)
        metrics = {}
...
if __name__ == '__main__':
    ...
    # # Configuration
    # dataset_id = 1  # Example dataset ID
    # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    base_params = {
        ...
        'num_epochs': 50,
        'batch_size': 32,
        'num_batches': len(np.argwhere(G_train == 1)) // 32,
        'learning_rate': 1e-3,
        'negative_ratio': 200
    }
    
    # Run experiments with different contrastive objectives
    results = run_contrastive_experiments(
        dataset_id=dataset_id,
        embeddings=embeddings,
        adj_matrix=G_train,
        base_params=base_params,
    ...
    save_dir = f'./results/contrastive_objectives/DS{dataset_id}'
    plot_contrastive_results(results, save_dir)
    
    # Print best configuration
    best_result = max(results, key=lambda x: x['metrics']['auc_roc'])
    print("\nBest Contrastive Objective Configuration:")
    print("Loss Type:", best_result['config']['loss_type'])
    print("Parameters:", best_result['config']['params'])
    ...
    # Optional: Cross-validation on validation set
    # valid_metrics = evaluate_contrastive_model(
    #     model, embeddings, G_valid, best_result['config']['loss_type'], device
    # )
    # print("\nValidation Set Performance:")
    # print_evaluation_results(valid_metrics)
